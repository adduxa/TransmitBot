 ...
 */
public class CreateReceiverSetTags extends CommandActivity {
	...
	public CreateReceiverSetTags(IUserActivity previousActivity, TelegramModule module, Long chatId) {
	...
	public void pushCommandUpdate(TelegramTransmitController controller, Update update) {
		controller.getTelegramModule().replyTo("Enter tags separated by commas that the receiver will listen to, or '*' to listen all messages\nIt can be any word contained in the target message. Usually hashtags\nFor ex. #transmit, #tm, #bot", update.getMessage());
	}
	...
	public void summon(TelegramTransmitController controller, Update update) {
		switch(state) {
			case ListeningTags: {
				...
				controller.getTelegramModule().replyTo("Enter tags separated by commas that the receiver will IGNORE\nIt can be any word contained in the target message. Usually hashtags\nFor ex. #notransmit, #notm, #nobot", update.getMessage());
			...
			case IgnoringTags: {
				...
				user.pushActivity(new CreateReceiverSetUsers(user.getActivity().getPreviousActivity(), module, chatId, transmitTags), controller, update);
				break;
			}
		}
	}
